...
#define SLE_ADV_MFG_MAGIC0                        0xAA
#define SLE_ADV_MFG_MAGIC1                        0xBB
#define SLE_ADV_MFG_NODE_TAG                      0x21
/* 广播名称 */
...
static void sle_refresh_local_name_by_addr(void)
{
    ...
    (void)memset_s(sle_local_name, NAME_MAX_LENGTH, 0, NAME_MAX_LENGTH);

    (void)snprintf_s((char *)sle_local_name,
                     NAME_MAX_LENGTH,
                     NAME_MAX_LENGTH - 1,
                     "s%02X%02X",
                     local_addr.addr[SLE_ADDR_LEN - 2],
                     local_addr.addr[SLE_ADDR_LEN - 1]);
}
...
static uint16_t sle_set_adv_data(uint8_t *adv_data)
{
    ...
    if (sle_get_local_addr(&local_addr) == ERRCODE_SLE_SUCCESS) {
        uint8_t mfg_value[5] = {
            ...
            local_addr.addr[SLE_ADDR_LEN - 2],
            local_addr.addr[SLE_ADDR_LEN - 1],
            SLE_ADV_MFG_NODE_TAG,
        };

        if (idx + 2 + sizeof(mfg_value) <= SLE_ADV_DATA_LEN_MAX) {
            adv_data[idx++] = (uint8_t)(1 + sizeof(mfg_value));
            adv_data[idx++] = SLE_ADV_DATA_TYPE_MANUFACTURER_SPECIFIC_DATA;
            ret = memcpy_s(&adv_data[idx], SLE_ADV_DATA_LEN_MAX - idx, mfg_value, sizeof(mfg_value));
            if (ret != EOK) {
                sample_at_log_print("%s mfg data memcpy fail\r\n", SLE_UART_SERVER_LOG);
                return 0;
            }
            idx += sizeof(mfg_value);
        }
    }
...
static uint16_t sle_set_scan_response_data(uint8_t *scan_rsp_data)
{
    ...
    sle_refresh_local_name_by_addr();
...
static int sle_set_default_announce_param(void)
{
    ...
    if (sle_get_local_addr(&local_addr) == ERRCODE_SLE_SUCCESS) {
        ...
        /* Fallback to non-random static address if stack cannot provide local address yet. */
        uint8_t fallback_addr[SLE_ADDR_LEN] = {0x01, 0x00, 0x00, 0x00, 0x00, 0x00};
        param.own_addr.type = 0;
        ret = memcpy_s(param.own_addr.addr, SLE_ADDR_LEN, fallback_addr, SLE_ADDR_LEN);
        if (ret != EOK) {
            sample_at_log_print("%s sle_set_default_announce_param fallback memcpy fail\r\n", SLE_UART_SERVER_LOG);
            return 0;
        }
    }
